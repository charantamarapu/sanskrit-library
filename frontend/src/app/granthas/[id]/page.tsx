...
export default function GranthaViewPage() {
  ...
  // Sidebar width and content styling
  const sidebarWidth = sidebarExpanded ? 'w-80' : 'w-16';
  const sidebarTransition = 'transition-all duration-300';
  const showContent = sidebarExpanded;
  ...
  return (
    <div className="h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        <aside className={`${sidebarWidth} bg-white border-r border-gray-200 flex flex-col ${sidebarTransition}`}>
          ...
          <div className="flex justify-between items-center p-4 border-b border-gray-200">
            ...
            <button
              ...
              className="ml-auto w-9 h-9 flex items-center justify-center rounded hover:bg-gray-100 transition"
              tabIndex={0}
              aria-label={sidebarExpanded ? 'Collapse sidebar' : 'Expand sidebar'}
            >
              <span className="inline-block text-xl">
                {/* Arrow icon switches direction on collapse */}
                {sidebarExpanded 
                  ? <svg width="20" height="20" fill="none" className="text-gray-900" viewBox="0 0 20 20"><path d="M14 10L8 4v12l6-6z" fill="currentColor"/></svg>
                  : <svg width="20" height="20" fill="none" className="text-gray-900" viewBox="0 0 20 20"><path d="M6 10l6 6V4l-6 6z" fill="currentColor"/></svg>
                }
              </span>
            </button>
          </div>
          ...
          {/* Sidebar Content */}
          <div className={`flex-1 overflow-y-auto p-4 ${!showContent ? 'p-0 flex flex-col items-center justify-center' : ''}`}>
            {showContent ? (
              <>
              ...
              <span className="rotate-90 text-xs text-gray-400">Filter</span>
            )}
          </div>

          {/* Sidebar Footer */}
          <div className={`p-4 border-t border-gray-200 space-y-3 ${sidebarTransition}`}>
            {showContent && (
              <>
                {/* Progress Indicator */}
